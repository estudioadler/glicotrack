"use client"

import Image from "next/image"
import Link from "next/link"
import { useState } from "react"

import { Button } from "@/components/ui/button"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { Input } from "@/components/ui/input"
import { Github01Icon, Radar01Icon } from "hugeicons-react"
import { Separator } from "@/components/ui/separator"
import { Label } from "@/components/ui/label"
import { signIn } from "@/auth"

export default function MarketplaceLogin() {
  const [email, setEmail] = useState("")
  const [isLoading, setIsLoading] = useState(false)

  const resendAction = (formData: FormData) => {
    signIn("resend", formData)
  }

  const handleMagicLinkLogin = async (e: React.FormEvent) => {
    e.preventDefault()
    setIsLoading(true)
    // Implement your magic link login logic here
    console.log("Magic link login for:", email)
    setIsLoading(false)
  }

  return (
    <div className="flex gap-4 min-h-screen p-6">
      {/* Left Section */}
      <div className="relative hidden  bg-muted lg:block w-3/5">
        <div className="px-8 py-4">
        <Link href="/dashboard" className="flex items-center gap-2">
          <Radar01Icon size={24} className="text-primary scale-x-[-1] -rotate-[225deg]" />
          <span className="uppercase font-semibold">Glicotrack</span>
        </Link>
        </div>
        <div className="relative mx-auto w-full flex items-center justify-center">
          <Image
            src="/Background.png"
            alt="Line login illustration"
            width={500}
            height={500}
            className="w-full"
            priority
          />
        </div>
      </div>

      {/* Right Section */}
      <div className="flex w-full items-center justify-center bg-muted lg:w-2/5">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1">
          <CardTitle className="text-2xl font-bold">Login</CardTitle>
          <CardDescription>Escolha seu método de login preferido.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <form onSubmit={handleMagicLinkLogin}>
            <div className="space-y-2">
              <Label htmlFor="email">Email</Label>
              <Input
                id="email"
                type="email"
                placeholder="m@example.com"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                required
              />
            </div>
            <Button onClick={() => resendAction(new FormData())} className="w-full mt-4" type="submit" disabled={isLoading}>
              {isLoading ? "Enviando..." : "Login com Magic Link"}
            </Button>
          </form>
          <div className="relative">
            <div className="absolute inset-0 flex items-center">
              <Separator className="w-full" />
            </div>
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-background px-2 text-muted-foreground">Ou continue com</span>
            </div>
          </div>
          <div className="flex flex-col gap-4">
            <Button variant="outline" onClick={() => signIn("google", { callbackUrl: "/dashboard" })}>
              <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24">
                <path
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                  fill="#4285F4"
                />
                <path
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  fill="#34A853"
                />
                <path
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                  fill="#FBBC05"
                />
                <path
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  fill="#EA4335"
                />
                <path d="M1 1h22v22H1z" fill="none" />
              </svg>
              Google
            </Button>
            <Button variant="outline" onClick={() => signIn("github", { callbackUrl: "/dashboard" })}>
              <Github01Icon className="w-5 h-5 mr-2" />
              GitHub
            </Button>
          </div>
        </CardContent>
      </Card>
      </div>
    </div>
  )
}

